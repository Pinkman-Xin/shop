<template>
  <div>
    ...
    <el-tab-pane label="静态属性" name="only">
      <el-button :disabled="isBtnDisabled" size="mini" type="primary" @click="addDialogVisible = true">添加属性</el-button>
...
export default {
  ...
  methods:{
    ...
    async showEditDialog(attr_id){
      ...
      const{data:res} = await this.$http.get(`categories/${this.cateId}/attributes/${attr_id}`,{params:{sel:this.activeName}})
      if (res.meta.status !==200) {
    ...
    showInput(row){
      ...
      // $nexTick方法作用当页面上元素被重新渲染过后才会指定回调函数中的代码
      this.$nextTick(_ => {
